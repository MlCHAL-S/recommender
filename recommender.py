...
def main():
    ...
    movie_stats = calculate_stats_and_bayesian_average(ratings)
    ...
    updated_ratings = update_ratings_with_bayesian_avg(ratings, bayesian_avg_ratings)

    sparse_matrix, user_to_index, movie_to_index, index_to_user, index_to_movie = create_sparse_matrix(updated_ratings)

    movie_titles = dict(zip(movies['movieId'], movies['title']))

    movie_id = 1  # Example movie ID
    similar_movies = find_similar_movies(movie_id, sparse_matrix, movie_to_index, index_to_movie, metric='cosine', k=10)
